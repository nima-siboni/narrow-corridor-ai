...
def Bellmann_iteration(n, pi, r, A, v):
    ...
    for i in range(0, n):
        for j in range(0, n):
            ...
            new_v[i, j] = r[i, j] # this part is assumed to be only dependent the state and the action 
            for action_id in range(0, nr_actions): #iterate over all possible actions
                ...
                if iprime >= 0 and iprime < n and jprime >= 0 and jprime < n : # if the action is allowed 
                    if (action_id == preferred_action_id):
                        ...
                        probability = 0.1 # this is not right! the some of probabilities should add-up to 1
...
def Q_estimation_for_state_s(i, j, A, gamma, v, candidate_action_id): #state s is defined by (i, j)
    ...
    for k in range(0, nr_actions): #iterate over all possible outcome
        action_for_kth_outcome = A[k]
        iprime, jprime = np.array([i, j]) + action_for_kth_outcome
        if iprime >= 0 and iprime < n and jprime >= 0 and jprime < n : # if the action is allowed 
